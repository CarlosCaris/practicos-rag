...
class QdrantManager:
    def __init__(self, collection_name="my_documents", data_path="data/processed"):
        """
        Clase para gestionar Qdrant usando t5-small para embeddings y búsquedas.
        ...
        self.embedding_model = AutoModelForSeq2SeqLM.from_pretrained("t5-small")
    ...
    def load_data(self):
        ...
        self.qdrant_vector_store = QdrantVectorStore.from_documents(
            docs,
            embedding=self.embed_documents,
            location=":memory:",
    ...
    def embed_documents(self, texts):
        """Crea embeddings usando t5-small."""
        inputs = self.tokenizer(texts, padding=True, truncation=True, return_tensors="pt")
        with torch.no_grad():
            embeddings = self.embedding_model.encoder(**inputs).last_hidden_state.mean(dim=1).numpy()
        return embeddings
