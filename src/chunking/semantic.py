...
def fn_semantic_chunk(documents, nlp_model, initial_chunk_size=512, min_length=100):
    """
    Realiza una segmentación semántica basada en oraciones tras una segmentación inicial.
    ...
    for document in documents:
        # Crear fragmentos iniciales y limpiar texto
        initial_chunks = text_splitter.create_documents([fn_clean_text(document.page_content)])

        for split in initial_chunks:
            split.metadata = document.metadata

            # Segmentación semántica basada en oraciones
            doc = nlp_model(split.page_content)
            ...
            for sent in doc.sents:
                ...
                if current_length + sent_length > min_length:
                    ...
                    current_chunk, current_length = [sent_text], sent_length
                else:
                    current_chunk.append(sent_text)
                    current_length += sent_length

            # Guardar último chunk si queda contenido
            if current_chunk:
                chunks.append(Document(
                    page_content=" ".join(current_chunk),
                    metadata=split.metadata
...
# Función para cargar el modelo spaCy
...
def fn_clean_text(text):
    """
    Limpia el texto eliminando caracteres especiales, acentos y stop words.
    ...
    return: 
        - Texto limpio.
    """
    text = unidecode(re.sub(r"[^a-záéíóúñü\s]", "", text.lower()))
    doc = nlp(text)
